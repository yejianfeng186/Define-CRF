...
def mapSDS(dfCRF,dfSDS,dfSdsDic,dfIgCdash,dfSanofiCdash,dfIgSdtm,dfSanofiSdtm):
    ...
    # dfMapSdtmType.to_csv("bbb.csv");
    # dfMapSdtmType["SDTMDOM"]=dfMapSdtmType["SDTMDOM"].apply(lambda x: ";".join(x) if isinstance(x,list) else "");
    ...
    # dfSdtmDomNoDup=dfSdtmDomMergeDes.drop_duplicates(["X","Y","CRFVAR","CDASHDOM","CDASHVAR","SDTMDOM","SDTMVAR"],keep="first");
    
    # dfSdtmDomAgg=dfSdtmDomNoDup.groupby(["X","Y"])["CDASHDOM","CDASHVAR","SDTMDOM","SDTMVAR"].agg(list);
    
    # dfSdtmDomAgg=dfSdtmDomNoDup.fillna('').groupby("level_0",as_index=True).agg(";".join);
    # dfSdtmDomJoin=dfMapSdtmType.drop(columns=["SDTMDOM"]).join(dfSdtmDomMergeDes);
    # print(dfSdtmDomJoin);
    # dfMapSdtmType.to_csv("bbb.csv");
    ...
    # dfAllSort=dfMapSdtmType.sort_values(by=["PAGENUMBER","Y","ORDER","OBJRANK"],ascending=[True,False,True,True]);
    dfAllSort=dfMapSdtmType.sort_values(by=["PAGENUMBER","Y","X"],ascending=[True,False,True]);
