...
def parseSDS(strFilePath,strFileName):
    ...
    dfSubForm=dfForm.loc[:,["OID","DraftFormName"]];
    dfSubFields=dfFields.loc[:,["FormOID","DraftFieldName","PreText","Ordinal","DataDictionaryName","DraftFieldActive","DefaultValue"]];
...
def mapSDS(dfCRF,dfSDS,dfSdsDic,dfIgCdash,dfSanofiCdash,dfIgSdtm,dfSanofiSdtm):
    ...
    dfStdAllSet=dfNodupVeriable.loc[:,["PAGENUMBER","ORDER","CLASS","X","Y","HEIGHT","WIDTH","CRFDS","CRFDSLAB","CRFVAR","CRFDES","CRFDIC","CRFDOM","CDASHTYPE","CRFVAL"
]];
...
if __name__ == '__main__':
    ...
    if fileProfile :
        ...
        # print(time.time());
        parseCRF=(strCRFPath,strCRFName);
