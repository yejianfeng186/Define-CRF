...
def parseCompanyCdash(strFilePath, strFileName):

    fileCompanyCdash = open(strFilePath + "\\std\\" + strFileName,
                            'r',
                            encoding='utf-8')
    jsonCompanyCdash = json.load(fileCompanyCdash)
    dfCompanyCdash = pd.DataFrame(jsonCompanyCdash)
    return dfCompanyCdash
...
def parseCompanySdtm(strFilePath, strFileName):
    fileCompanySdtm = open(strFilePath + "\\std\\" + strFileName,
                           'r',
                           encoding='utf-8')
    jsonCompanySdtm = json.load(fileCompanySdtm)
    dfCompanySdtm = pd.DataFrame(jsonCompanySdtm)
    return dfCompanySdtm


def parseCompanySdtmDom(strFilePath, strFileName):
    fileCompanySdtmDom = open(strFilePath + "\\std\\" + strFileName,
                              'r',
                              encoding='utf-8')
    jsonCompanySdtmDom = json.load(fileCompanySdtmDom)
    dfCompanySdtmDom = pd.DataFrame(jsonCompanySdtmDom)
    return dfCompanySdtmDom



def mapSDS(dfCRF, dfSDS, dfSdsDic, strCompany, dfIgCdash, dfCompanyCdash,
           dfIgSdtm, dfCompanySdtm, boolControlVersion):
    dfCrfQue = dfCRF[dfCRF["class"] == "question"].copy()
    dfCrfQue["CDASHTYPE"] = strCompany
    ...
    dfMapIgCdash = pd.merge(dfMapSdsDic,
                            ...
                            right_on="CDASHVAR",
                            how="left")
    strCompanyCdashVar=strCompany+"CDASHVAR"
    strCompanySdtmDom=strCompany+"SDTMDOM"
    strCompanySdtmVar=strCompany+"SDTMVAR"

    dfCompanyCdashRename = dfCompanyCdash.rename(
        columns={
            "CDASHVAR": strCompanyCdashVar,
            "SDTMDOM": strCompanySdtmDom,
            "SDTMVAR": strCompanySdtmVar
        })
    dfMapCompanyCdash = pd.merge(dfMapIgCdash,
                                dfCompanyCdashRename,
                                left_on="CRFVAR",
                                right_on=strCompanyCdashVar,
                                how="left")
    dfMapCdashType = dfMapCompanyCdash.apply(setCdashType,args=(strCompany,),axis=1).drop(
        columns=[strCompanyCdashVar, strCompanySdtmDom, strCompanySdtmVar]).drop(
            columns=["CDASHQUE"])
    ...
    dfCompanySdtmRename = dfCompanySdtm.rename(columns={
        "SDTMDOM": strCompanySdtmDom,
        "SDTMVAR": strCompanySdtmVar
    })
    dfMapCompanySdtm = pd.merge(dfMapIgSdtm,
                               dfCompanySdtmRename,
                               left_on="CRFVAR",
                               right_on=strCompanySdtmVar,
                               how="left")

    dfMapSdtmType = dfMapCompanySdtm.apply(setSdtmType,args=(strCompany,), axis=1).drop(
        columns=["IGSDTMDOM", "IGSDTMVAR", strCompanySdtmDom, strCompanySdtmVar])
...
def setCdashType(var,strCompany):

    strCompanyCdashVar=strCompany+"CDASHVAR"
    strCompanySdtmDom=strCompany+"SDTMDOM"
    strCompanySdtmVar=strCompany+"SDTMVAR"

    if not isinstance(var[strCompanySdtmVar], list):
        var[strCompanySdtmVar] = []
    ...
    if pd.notnull(var["CDASHVAR"]):
        ...
        if len(var["SDTMVAR"]) > 0:
            ...
            # if pd.notnull(var[strCompanySdtmVar]):
            if len(var[strCompanySdtmVar]) > 0:
                var["SDTMVAR"] = var[strCompanySdtmVar]
                var["SDTMDOM"] = var[strCompanySdtmDom]

        elif len(var[strCompanySdtmVar]) > 0:
            var["SDTMTYPE"] = strCompany
            var["SDTMVAR"] = var[strCompanySdtmVar]
            var["SDTMDOM"] = var[strCompanySdtmDom]

    elif pd.notnull(var[strCompanyCdashVar]):
        # elif var[strCompanyCdashVar].length>0:
...
def setSdtmType(var,strCompany):
    strCompanySdtmDom=strCompany+"SDTMDOM"
    strCompanySdtmVar=strCompany+"SDTMVAR"
    if not isinstance(var["SDTMVAR"], list):
        ...
        elif pd.notnull(var[strCompanySdtmVar]):
                ...
                for q, q_row in seriousTargetVeriable.iterrows():
                    if (q_row["CRFDES"] in p_row["pretext"]) or (
                        ...
                        if q_row["CRFDES"].find(p_row["pretext"]) == 0:
                            ...
                            break
            # dfLegacyCrf.to_csv("aaa.csv");
            if  'CLASS' in dfLegacyCrf.columns:
...
if __name__ == '__main__':
    ...
    if fileProfile:
        ...
        strCompany = jsonInfo["CompanyName"]
        ...
        dfCompanyCdash = pd.DataFrame(dfMapping[strCompany+"CDASH"])
        dfIgSdtm = pd.DataFrame(dfMapping["IGSDTM"])
        dfCompanySdtm = pd.DataFrame(dfMapping[strCompany+"SDTM"])
        ...
        for strSdsFullName in arrSdsFullName:
            ...
            if not dfCRF.empty:
                dfACRF = mapSDS(dfCRF, dfSDS, dfSdsDic, strCompany,dfIgCdash,
                                dfCompanyCdash, dfIgSdtm, dfCompanySdtm,
                                boolControlVersion)
