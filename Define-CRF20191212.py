...
def parseCDASH(strFilePath,strFileName):
    ...
    dfSubCDASHRename=dfSubCDASHNotNull.rename(columns={"Question Text":"SQuestion","SDTMIG Target":"SDTMIG","CDASHIG Variable":"CDASHIG"});
    dfSubCDASHNodupe=dfSubCDASHRename.drop_duplicates(keep="first");
    # print("CDASH finished Loading");
...
def parseSanofiMapping(strFilePath,strFileName):
    ...
    dfSubField=dfField.loc[:,["CDASHSANOFI","SDTMSANOFI"]];
...
def parseSanofiSDTM(strFilePath,strFileName):
    ...
    dfSubField=dfField.loc[:,["Variable Name"]];
    dfSubFieldRename=dfSubField.rename(columns={"Variable Name":"GSDTMSANOFI"});
    
    dfSubFielNotNull=dfSubFieldRename[dfSubFieldRename["GSDTMSANOFI"].notnull()];
    dfSubFielNodupe=dfSubFielNotNull.drop_duplicates(keep="first");
    # print("SANOFI standard end Loading");
    return dfSubFielNodupe;
...
def mapSDS(dfCRF,dfSDS,dfDictionary,dfCDASH,dfSanofiSDTM,dfSanofiMapping):
    ...
    # dfMapSANOFI=pd.merge(dfMapCDASH,dfSanofiSDTM,left_on="CDASH",right_on="GSDTMSANOFI",how="left");
    ...
    dfMapSANOFIType=dfMapSANOFICDASH.apply(setSDTMType, axis=1);
    dfMapSANOFIFilter=dfMapSANOFIType[["pagenumber","form","order","PreText","SQuestion","CDASH","CDASHTYPE","SDTM","SDTMTYPE","codename","class","x","y","height","width"]];
...
def createAnnotation(dfACRF,strFilePath,strFileName,strCurrentDatetime,strType):
    ...
    for i,i_row in dfSubACRF.iterrows():
        ...
        if str.upper(strType) == "CDASH" or str.upper(strType) == "BOTH" :
        ...
        if pd.notnull(i_row.SDTM) and i_row.SDTM !="" and (str.upper(strType) == "SDTM" or str.upper(strType) == "BOTH"):
            # print(i_row.SDTM);
            # print(i_row.CDASH);
...
if __name__ == '__main__':
    ...
    fileProfile = open("init.json",'r',encoding='utf-8');
    ...
    if fileProfile :
        ...
        # createAnnotation(dfACRF,strCRFName,strType);
        createAnnotation(dfACRF,strCRFPath,strCRFName,strCurrentDatetime,strType);
        ...
        time2 = time.time();
        print(strFilename+strCurrentDatetime+".json");
